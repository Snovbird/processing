...
import tempfile
import os
from common.common import select_video,askint,windowpath,clear_gpu_memory

def combine_videos_with_cuda(input_files):
    """
    ...
    outputfile = f'concat{os.path.basename(input_files[0])[0]}.mp4'
    output = os.path.join(os.path.dirname(input_files[0]),outputfile)
...
def main():
    ...
    toconcat = [select_video(title="select videos to concatenate first",chosenpath=startpath) for c in range(number_of_concats)]
    for setofvids in toconcat:
        ready = combine_videos_with_cuda(setofvids)
    if ready:
        os.startfile(os.path.dirname(toconcat[0][0]))
