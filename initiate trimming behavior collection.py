...
def clips_overlay_missing(path = find_folder_path("5-clips"),DS = "DS-"):
    collectedpath = os.path.join(path,f"collected {DS}")
    original_clips = list_files(os.path.join(collectedpath,"originals"))

    currently_overlaid = [i.replace("-overlaid","") for i in list_files(collectedpath)]

    donepath = find_done_folder(path,DS)
    for i in original_clips:
        if i not in currently_overlaid and i not in donepath:
            overlay_FRAMES(os.path.join(collectedpath,"originals",i),collectedpath)
